    ...
    <div className="flex h-screen bg-gray-50 font-sans text-gray-800 overflow-hidden">
      ...
      <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 w-[280px] fixed lg:relative h-screen lg:h-auto transition-transform duration-300 z-40 lg:z-auto flex flex-col bg-gradient-to-b from-purple-900 via-violet-800 to-purple-900 text-white py-6 px-4 shadow-2xl overflow-y-auto shrink-0`}>
        <div className="flex items-center gap-3 mb-8 pb-5 border-b border-white/10">
          ...
          <h1 className="text-lg font-bold bg-gradient-to-r from-purple-300 to-violet-300 bg-clip-text text-transparent">Messages Hub</h1>
        </div>

        <div className="flex items-center gap-3 mb-8 p-4 rounded-xl bg-white/5 border border-white/10">
          <div className="w-11 h-11 rounded-lg bg-gradient-to-br from-purple-400 to-violet-600 flex items-center justify-center text-white font-bold text-lg shadow-lg shadow-purple-500/30">
        ...
        <nav className="flex flex-col gap-1 flex-1">
          <button onClick={() => navigate("/coordination-dashboard")} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-300 hover:bg-white/10 hover:text-white transition-all">
            <LayoutDashboard size={18} /> Dashboard
          </button>
          <button className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium bg-gradient-to-r from-purple-500 to-violet-600 text-white shadow-lg shadow-purple-500/20">
            <MessageSquare size={18} /> Messages
          </button>
          <button onClick={() => navigate("/coordination-edit-profile")} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-300 hover:bg-white/10 hover:text-white transition-all">
            <UserPen size={18} /> Edit Profile
          </button>
          <button onClick={() => navigate("/coordination-dashboard")} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-300 hover:bg-white/10 hover:text-white transition-all">
            <ThumbsDown size={18} /> Reject
          </button>
          <button onClick={() => navigate("/coordination-issue-return")} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-300 hover:bg-white/10 hover:text-white transition-all">
      ...
      <main className="flex-1 overflow-y-auto p-8">
        <div className="mb-8 rounded-2xl bg-gradient-to-br from-purple-50 to-violet-50 border border-purple-100 shadow-sm p-6 flex items-center justify-between">
        ...
        <div className="mb-8 flex items-center gap-2 bg-white rounded-lg p-2 shadow-sm border border-gray-200 flex-wrap">
          <button onClick={() => { setActiveTab("received"); setSearchQuery(""); }} className={`flex items-center gap-2 px-4 py-2.5 rounded-md text-sm font-semibold transition-all ${activeTab === "received" ? "bg-gradient-to-r from-purple-500 to-violet-600 text-white shadow-lg" : "text-gray-600 hover:bg-gray-100"}`}><Inbox size={16} /> Received ({filteredReceived.length})</button>
          <button onClick={() => { setActiveTab("send"); setSearchQuery(""); setError(""); }} className={`flex items-center gap-2 px-4 py-2.5 rounded-md text-sm font-semibold transition-all ${activeTab === "send" ? "bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-lg" : "text-gray-600 hover:bg-gray-100"}`}><Send size={16} /> Send</button>
