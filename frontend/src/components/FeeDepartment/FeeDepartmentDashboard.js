...
import { 
  ...
  DollarSign, AlertCircle, CheckCircle, Inbox, AlertTriangle, Menu
} from "lucide-react";
...
export default function FeeDepartmentDashboard() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      console.log("🔄 Auto-refreshing fee department dashboard data...");
      fetchRequests();
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-gray-50 font-sans text-gray-800 overflow-hidden">
      ...
      {/* ── SIDEBAR ── */}
      <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:relative w-[280px] h-screen lg:h-auto flex flex-col bg-gradient-to-b from-[#5a2e0f] via-[#8b4513] to-[#654321] text-white py-6 px-4 shadow-xl overflow-y-auto shrink-0 transition-transform duration-300 z-40 lg:z-auto`}>
        {/* Logo & Title */}
        ...
        <div className="flex items-center gap-3 mb-8 p-4 rounded-xl bg-white/5 border border-white/10">
          ...
          <div className="min-w-0">
            <h3 className="text-sm font-semibold truncate">{user?.full_name || "Fee Department"}</h3>
            <p className="text-[11px] text-gray-300 truncate">{user?.sap || "N/A"} • Fee Dept</p>
          </div>
        </div>
        ...
        <nav className="flex flex-col gap-1 flex-1">
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 ${activeTab === "approved" ? "bg-gradient-to-r from-orange-500 to-amber-600 text-white shadow-lg shadow-orange-500/20" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}
          >
            <CheckCircle2 size={18} /> Approved
          </button>
          <button
            onClick={() => setActiveTab("rejected")}
            className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 ${activeTab === "rejected" ? "bg-gradient-to-r from-orange-500 to-amber-600 text-white shadow-lg shadow-orange-500/20" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}
          >
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-8 mt-14 lg:mt-0">
        {/* Hero Header Card */}
        <div className="mb-8 rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-100 shadow-sm p-6">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Fee Department</h1>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-gray-400">
            <div className="animate-spin mb-4"><ClipboardList size={32} className="text-orange-400" /></div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {requests.map((req, index) => (
                    <tr key={req._id || req.id} className="hover:bg-blue-50/30 transition-colors duration-150">
                      ...
                      </td>                      <td className="px-5 py-4 text-center flex items-center justify-center gap-2">
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 border border-gray-200 animate-[fadeIn_0.3s_ease]">
              ...
              <div className="mb-6">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200 placeholder:text-gray-400 resize-none"
                  rows={4}
                />
              </div>
