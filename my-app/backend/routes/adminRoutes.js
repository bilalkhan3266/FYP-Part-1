...
router.get('/department-stats', verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    // Get real-time stats directly from DepartmentClearance collection using aggregation pipeline
    const submissionStats = await DepartmentClearance.aggregate([
      {
        $group: {
          _id: '$department_name',
          totalRequests: { $sum: 1 },
...
/**
 * POST /api/admin/auto-approve-pending
 * Batch auto-approve all pending clearance requests
 * ADMIN ONLY - Use with caution!
 */
router.post('/auto-approve-pending', verifyToken, verifyAdmin, async (req, res) => {
  try {
    console.log('🤖 BATCH AUTO-APPROVE: Starting process to auto-approve all pending requests...');
    ...
    if (pendingRequests.length === 0) {
      return res.json({
        success: true,
        message: '✅ No pending requests found. System is clean!',
        data: {
          totalProcessed: 0,
          approved: 0,
          byDepartment: {}
    ...
    for (const request of pendingRequests) {
      try {
        ...
        console.log(`✅ Auto-approved: ${request.sapid} - ${dept}`);
      } catch (err) {
        console.error(`❌ Failed to auto-approve ${request._id}:`, err.message);
      }
    }

    console.log(`🤖 BATCH AUTO-APPROVE COMPLETE: ${approvedCount} requests approved`);

    res.status(200).json({
      success: true,
      message: `✅ Auto-approved ${approvedCount} pending requests!`,
      data: {
    ...
    console.error('Auto-Approve Pending Error:', error);
    res.status(500).json({
      success: false,
      message: '❌ Failed to auto-approve pending requests',
      error: error.message
    });
  }
});
